...
export const sortUsersByAverageScore_descendingOrder = (users) =>
    users.sort((userA, userB) => {
        ...
        if (userA.ratingsByOthers.length != 0) userA_scoreAverage = userA_score / userA.ratingsByOthers.length;

        const userB_score = userB.ratingsByOthers.reduce(getUserTotalScore, 0);
        let userB_scoreAverage = 0;
        if (userB.ratingsByOthers.length != 0) userB_scoreAverage = userB_score / userB.ratingsByOthers.length;
